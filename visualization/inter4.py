...
for i in range(len(merged_data)):
    ...
    plt.text(x=merged_data['Shannon Entropy'][i],
             y=merged_data['Group Frequency'][i],
             s=construct_name,
             fontsize=8,
             ha='right',
...
plt.text(x=0, y=frequency_threshold, s=f'Median Group Frequency: {frequency_threshold:.2f}', color='grey', fontsize=10, va='bottom', ha='left')
plt.text(x=entropy_threshold, y=min(merged_data['Group Frequency']), s=f'Median Shannon Entropy: {entropy_threshold:.2f}', color='grey', fontsize=10, va='bottom', ha='right', rotation=90)
